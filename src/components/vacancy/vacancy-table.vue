...
import { convertDate, langs } from '@/stores/env'
...
<template>
  <el-table stripe :data="data">
    ...
    <el-table-column label="Время приёма" width="150">
      <template #default="scope">
        {{ convertDate(scope.row.from) }} -
        {{ convertDate(scope.row.to) }}
      </template>
    </el-table-column>
    <el-table-column label="Ссылка" width="100">
      <template #default="scope">
        <a :href="scope.row.link" target="_blank"> Ссылка </a>
      </template>
    </el-table-column>
